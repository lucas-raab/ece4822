...
for (int j = 0; j < nn->input_size; j++) {
    ...
    for (int i = 0; i < nn->hidden_size; i++) {
        nn->hidden_output[i] = sigmoid(nn->bias_hidden[i] + input[j] * nn->weights_input_hidden[j][i]);
    }
}
...
for (int j = 0; j < nn->hidden_size; j++) {
    ...
    for (int i = 0; i < nn->output_size; i++) {
        nn->predicted_output[i] = sigmoid(nn->bias_output[i] + nn->hidden_output[j] * nn->weights_hidden_output[j][i]);
    }
}
}
...
void backward(NeuralNetwork* nn, float* input, float* target, float learning_rate) {
    ...
    for (int i = 0; i < nn->output_size; i++) {
        output_delta[i] = (target[i] - nn->predicted_output[i]) * sigmoid_derivative(nn->predicted_output[i]);
    }
